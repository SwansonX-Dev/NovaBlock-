    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        switch (sub) {
            ...
            case "giveoneblock" -> {
                ...
                int amountArg = 2;
                if (args.length >= 3) {
                    com.nova.novablock.community.CommunityNodeType parsed = com.nova.novablock.community.CommunityNodeType.byId(args[2]);
                    if (parsed != null) { nodeType = parsed; amountArg = 3; }
                }
                if (args.length > amountArg) {
                    try { amount = Math.max(1, Math.min(64, Integer.parseInt(args[amountArg]))); }
                    catch (NumberFormatException ex) { Msg.send(sender, "<red>Amount must be a number."); return true; }
                }
                ItemStack item = com.nova.novablock.listener.OneBlockGrantListener.create(plugin, nodeType, amount);
